...
#define REALSENSE_ROS_EMBEDDED_VERSION_STR (VAR_ARG_STRING( \
    VERSION: REALSENSE_ROS_MAJOR_VERSION. \
...
void RealSenseNodeFactory::getDevice(rs2::device_list list)
{
    ...
    if (0 == list.size()) {
      ...
      for (size_t count = 0; count < list.size(); count++) {
        ...
        try {
          ...
          ROS_WARN_STREAM(
            "Device " << count + 1 << "/" << list.size() << " failed with exception: " <<
              ex.what());
          continue;
        }
        ...
        if ((_serial_no.empty() || sn == _serial_no) &&
          (_usb_port_id.empty() || port_id == _usb_port_id) && found_device_type)
        {
          _device = dev;
          _serial_no = sn;
          found = true;
      ...
      if (!found) {
        ...
        if (_device.supports(RS2_CAMERA_INFO_USB_TYPE_DESCRIPTOR)) {
          ...
          if (usb_type.find("2.") != std::string::npos) {
            ROS_WARN_STREAM(
              "Device " << _serial_no << " is connected using a " << usb_type <<
  ...
  bool remove_tm2_handle(_device && RS_T265_PID != std::stoi(
      _device.get_info(
...
void RealSenseNodeFactory::init()
{
  try {
    ...
    std::cin.get();
#endif
    _serial_no = declare_parameter(
      "serial_no",
      rclcpp::ParameterValue("")).get<rclcpp::PARAMETER_STRING>();
    _usb_port_id = declare_parameter(
      "usb_port_id",
      rclcpp::ParameterValue("")).get<rclcpp::PARAMETER_STRING>();
    _device_type = declare_parameter(
      "device_type",
      rclcpp::ParameterValue("")).get<rclcpp::PARAMETER_STRING>();

    // A ROS2 hack: until a better way is found to avoid auto convertion of strings containing only digits to integers:
    if (_serial_no.front() == '_') {
      _serial_no = _serial_no.substr(1);                                                // remove '_' prefix

    }
    std::string rosbag_filename(declare_parameter(
        "rosbag_filename", rclcpp::ParameterValue(
    ...
    if (!rosbag_filename.empty()) {
      ...
      _initial_reset = declare_parameter(
        "initial_reset",
        rclcpp::ParameterValue(false)).get<rclcpp::PARAMETER_BOOL>();
      ...
      _query_thread = std::thread(
        [ = ]()
        {
          ...
          while (_is_alive && !_device) {
            ...
            if (_device) {
              std::function<void(rs2::event_information &)> change_device_callback_function = [this](
                rs2::event_information & info) {changeDeviceCallback(info);};
...
void RealSenseNodeFactory::startDevice()
{
  ...
  try {
    switch (pid) {
      ...
      case RS_L515_PID:
        _realSenseNode =
          std::unique_ptr<BaseRealSenseNode>(new BaseRealSenseNode(*this, _device, _serial_no));
        break;
      case RS_T265_PID:
        _realSenseNode =
          std::unique_ptr<T265RealsenseNode>(new T265RealsenseNode(*this, _device, _serial_no));
        break;
      default:
